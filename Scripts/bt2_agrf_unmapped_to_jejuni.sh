...
#SBATCH --job-name Unmap_to_jej_agrf
...
READS_DIR="/nesi/nobackup/massey03212/Nat_comms/Genomic_analysis/15_mapping_experiment/10_C_jejuni_investigation/agrf_mapping/individual_mapped_results/unmapped_reads"
INDEX_BASE="/nesi/nobackup/massey03212/Nat_comms/Genomic_analysis/15_mapping_experiment/02_References/for_humans"
OUTPUT_DIR="${BASE_DIR}/unmapped_to_C_jejuni"
COUNT_FILE="${OUTPUT_DIR}/unmapped_to_jejuni_agrf.tsv"
...
# Find all unique sample IDs based on R1 files, assuming the consistent '.fastq.gz' suffix.
SAMPLES=$(find "${READS_DIR}" -maxdepth 1 -name '*_R1.fastq.gz' -type f | \
    xargs -n 1 basename | \
    sed 's/_R1.fastq.gz//' | \
    sort -u)

if [ -z "$SAMPLES" ]; then
    echo "❌ ERROR: No '.fastq.gz' files found in $READS_DIR. Exiting."
    exit 1
fi
...
for sample in $SAMPLES; do
    ...
    if [ ! -f "$R1" ] || [ ! -f "$R2" ]; then
        echo "❌ ERROR: Paired host-filtered files not found for sample $sample. Skipping."
        continue
    fi
