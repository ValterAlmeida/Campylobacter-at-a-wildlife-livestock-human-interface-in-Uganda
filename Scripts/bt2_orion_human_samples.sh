...
#SBATCH --job-name Campy_map_Orion
...
READS_DIR="${BASE_DIR}/orion_human_files" # Input FASTQ files are here (.fastq)
INDEX_BASE="/nesi/nobackup/massey03212/Nat_comms/Genomic_analysis/15_mapping_experiment/02_References/for_humans"
OUTPUT_DIR="${BASE_DIR}/individual_mapped_results_orion"
...
R1_SUFFIX="_hostFilt_R1.fastq"
R2_SUFFIX="_hostFilt_R2.fastq"
...
if [ -z "$SAMPLES" ]; then
    echo "❌ ERROR: No matching R1 files found in $READS_DIR. Exiting."
    exit 1
fi
...
for sample in $SAMPLES; do
    ...
    if [ ! -f "$R1" ] || [ ! -f "$R2" ]; then
        echo "❌ ERROR: Paired files not found for sample $sample. Skipping."
        continue
    fi
