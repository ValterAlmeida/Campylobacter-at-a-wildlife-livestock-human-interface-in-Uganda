---
title: "Genome-resolved metagenomics of Campylobacter at a wildlife–livestock–human interface in Uganda reveals novel species associated with humans in healthcare settings"
author:
  - Valter Almeida
  - Matthew A. Knox
  - Kim M. Handley
...
After taxonomic classification and multiple sequence alignment, a phylogenetic tree was inferred using IQ-TREE2 (v2.2.2.2). Branch support was assessed with 1000 ultrafast bootstrap replicates and the phylogenetic tree was visualised and annotated using the Interactive Tree of Life (iTOL v6). The *Helicobacter pylori* reference genome (strain CHC155; NCBI accession GCA_025998455.1) was included as an outgroup for phylogenetic analysis.
...
### Mapping to *Campylobacter* reference genomes
...
### Read mapping, re-mapping strategy, and taxonomic validation

The same reference genomes were used for both read- and contig-level mapping analyses. Host-filtered paired-end reads were mapped using Bowtie2 (v2.5.4) in combination with SAMtools (v1.21). Reference genomes included *C. jejuni* and *C. coli*, as well as the highest-quality human-associated *Campylobacter* MAGs assembled in this study. MAGs were selected based on high genome completeness and low contamination, as assessed during genome quality control.

#### Primary read mapping

Initial mapping was performed by aligning host-filtered reads against *C. jejuni* and *C. coli* reference genome indices. Bowtie2 was run in `--sensitive` mode with paired-end constraints (`--minins 200`, `--maxins 800`) to ensure accurate alignment of properly paired reads while allowing for insert size variability.

Unaligned paired reads were captured using the `--un-conc-gz` option, enabling downstream re-mapping steps. Only mapped reads were retained in BAM format by filtering out unmapped reads (`-F 4`) during SAMtools conversion. Mapped read counts for each species were calculated directly from BAM files using `samtools view -c`. Mapped reads were additionally extracted back into paired FASTQ format (`-f 3`) to allow downstream inspection and validation.

#### Secondary re-mapping strategy

To evaluate whether reads initially mapping to *C. jejuni* or *C. coli* could instead be explained by locally assembled *C. infans* or *C.* sp900539255 genomes, a two-step mapping strategy was applied. Reads that did not map to *C. infans* or *C.* sp900539255 MAGs were subsequently re-mapped to the *C. jejuni* and *C. coli* reference genomes. This approach allowed discrimination between true *C. jejuni*/*C. coli* signal and reads more accurately explained by closely related, locally assembled genomes. This iterative mapping framework reduced misassignment driven by reference bias and improved confidence in species-level attribution, particularly in the context of closely related *Campylobacter* taxa.

#### Assembly and BLAST-based validation of *C. jejuni*-mapped reads

Reads mapping to *C. jejuni* were assembled de novo using MEGAHIT (v1.2.9). Resulting contigs were queried against the NCBI nucleotide (nt) database using BLASTn (v2.16.0) with the RefSeq BLAST database release BLASTDB/2025-08.

BLASTn searches were performed using a stringent e-value threshold (1e-10), with up to five target sequences retained per query. Output was generated in tabular format (outfmt 6), reporting query and subject identifiers, percent identity, alignment length, query coverage, e-value, bitscore, taxonomic identifiers, and scientific names. This analysis enabled taxonomic validation of assembled contigs and assessment of whether sequences initially assigned to *C. jejuni* represented true *C. jejuni* genomic material or homologous regions shared with other *Campylobacter* species.
...
Functional and metabolic annotation was also performed using KofamScan (v1.3.0) to assign KEGG Orthology (KO) identifiers to the protein sequences of the assembled MAGs. The resulting data were then processed with KEGGDecoder (v1.3) to map KO identifiers to complete metabolic pathways.

KofamScan was executed with two primary output formats: a **mapper** format for KEGGDecoder input, and a **detail** format for comprehensive manual checking.

|  |  |
|------------------------------------|------------------------------------|
| **Parameter** | **Purpose and Explanation** |
| `-o` | **Output file name** for the generated annotation results. |
| `-E 0.0001` | Sets the **E-value threshold** for ortholog assignment, filtering out hits with lower statistical significance. |
| `-k` | Specifies the path to the **KO list database** (`ko\_list`) used by KofamScan. |
| `-p` | Specifies the path to the **HMM profile database** (`profiles`) of the KOfam HMM library. |
| `-f mapper` | Sets the output format to **"mapper,"** which provides a concise, two-column output optimised for input into KEGGDecoder. |
| `-f detail` | Sets the output format to **"detail,"** which provides comprehensive information for each hit, including alignment scores and e-values. |

\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_\_

KEGGDecoder (v1.3) was used to map the KofamScan output to metabolic pathways.

|  |  |
|------------------------------------|------------------------------------|
| **Parameter** | **Purpose and Explanation** |
| `-i` | **Input file**, which is the concise KofamScan output (`-f mapper`). |
| `-o` | **Output file name** for the resulting pathway data. |
| `-v static` | Specifies generation of the **static output** format, which provides a single summary file of pathway completeness. |
| `-v interactive` | Specifies generation of the **interactive output** format, allowing for dynamic visualisation of pathway coverage. |
